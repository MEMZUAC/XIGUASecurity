...
namespace Xdows.ScanEngine
{
    ...
    {
        public static async Task<(int score, string extra, string threatName)> Evaluate(string path, PeFile peFile, Xdows.ScanEngine.ScanEngine.PEInfo peInfo, bool deepScan)
        {
            ...
            else if (fileExtension == ".fne")
            {
                ...
                threatName = "HEUR:Elang.Component.a";
            }
            ...
            if (mediaExts.Any(mediaExt => path.Contains(mediaExt)))
            {
                ...
                threatName = "HEUR:Media.Suspicious.a";
            }
                 ...
                 path.Contains(@"\ProgramData\Microsoft\Windows\Start Menu\Programs\Startup\", StringComparison.OrdinalIgnoreCase)))
            {
                try
                {
                    ...
                    if ((attrs & FileAttributes.Hidden) != 0)
                    {
                        ...
                        threatName = "HEUR:Startup.Hidden.a";
                    }
                }
                catch { }
            }
            if (peFile.IsExe || peFile.IsDll || peFile.IsDriver) // .NET文件无法分析
            {
                ...
                if (tempScore > 0)
                {
                    ...
                    if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:PE.AbnormalRes.a";
                }
                ...
                if (tempScore > 0)
                {
                    ...
                    if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:PE.Exception.a";
                }
                ...
                if (await packingTask)
                {
                    ...
                    if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:PE.Packed.a";
                }
                if (peFile.IsDll)
                {
                    if (peInfo.ExportsName == null)
                    {
                        ...
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:DLL.Camouflage.a";
                    }
                    else
                    {
                ...
                {
                    score += 5;
                    if (string.IsNullOrEmpty(threatName)) threatName = peFile.IsDotNet ? "HEUR:NET.Suspicious.a" : "HEUR:DRV.Suspicious.a";
                }
                ...
                if (peInfo.ImportsName != null)
                {
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["LoadLibrary"]))
                    {
                        ...
                        {
                            score += 15;
                            if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.DynamicLoad.a";
                        }
                        else
                        {
                            score += 10;
                            if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.LibLoad.a";
                        }
                    }
                    ...
                    {
                        score += 20;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.FileHide.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["SHFormatDrive"]))
                    {
                        score += 20;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.FormatDisk.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["RtlAdjustPrivilege"]))
                    {
                        score += 20;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.PrivEsc.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["HideCurrentProcess"]))
                    {
                        score += 20;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.ProcHide.a";
                    }
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["SetFilePointer"]))
                    {
                        ...
                        if (deepScan && ContainsSuspiciousApi(peInfo.ImportsName, ["WriteFile"]) && ContainsSuspiciousContent(fileContent, ["physicaldrive0"]))
                        {
                            ...
                            threatName = "HEUR:API.MBRMod.a";
                        }
                        else if (string.IsNullOrEmpty(threatName))
                        {
                            threatName = "HEUR:API.FilePtr.a";
                        }
                    }
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["CreateService"]) && ContainsSuspiciousApi(peInfo.ImportsName, ["StartService"]))
                    {
                        ...
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.Service.a";
                    }
                    ...
                    {
                        score += 5;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.FileOps.a";
                    }
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["CreateObject"]))
                    {
                        ...
                        {
                            score += 15;
                            if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.ScriptObj.a";
                        }
                    }
                    ...
                    {
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.DlgInput.a";
                    }
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["InternetReadFile", "FtpGetFile", "URLDownloadToFile"]))
                    {
                        if (ContainsSuspiciousApi(peInfo.ImportsName, ["WinExec"]) && ContainsSuspiciousApi(peInfo.ImportsName, ["RegCreateKey"]))
                        {
                            ...
                            if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.NetDownload.a";
                        }
                        else if (string.IsNullOrEmpty(threatName))
                        {
                            threatName = "HEUR:API.NetDownload.b";
                        }
                    }
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["CallNextHookEx", "SetWindowsHook"]))
                    {
                        ...
                        score += 20;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.KeyHook.a";
                    }
                    else if (ContainsSuspiciousApi(peInfo.ImportsName, ["Hook"]))
                    {
                        suspiciousData.Add("LikeAddHook");
                        score += 10;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.Hook.a";
                    }
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["VirtualAlloc", "VirtualFree", "VirtualProtect", "VirtualQuery"]))
                    {
                        ...
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.MemMod.a";
                    }
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["GetModuleFileName", "GetModuleHandle"]))
                    {
                        score += 20;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.ModHandle.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["WNetAddConnection"]))
                    {
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.NetConn.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["CopyScreen"]))
                    {
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.ScreenCap.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["ExitWindows"]))
                    {
                        score += 5;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.WinExit.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["URLDownloadToFile"]))
                    {
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.URLDownload.a";
                    }
                    ...
                    {
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.InputSim.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["SetPriorityClass"]))
                    {
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.ProcPrio.a";
                    }
                    ...
                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["EnumAudioEndpoints"]))
                    {
                        ...
                        score += 15;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.SandBypass.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["AdjustTokenPrivileges"]))
                    {
                        score += 10;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.TokenPriv.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["CreateRemoteThread"]))
                    {
                        score += 10;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.RemoteThread.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["InternetOpen", "InternetConnect", "HttpSendRequest"]))
                    {
                        score += 5;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.HTTP.a";
                    }

                    if (ContainsSuspiciousApi(peInfo.ImportsName, ["RegCreateKey", "RegSetValue"]))
                    {
                        score += 5;
                        if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:API.RegMod.a";
                    }
                }
            }
            ...
            if (deepScan)
            {
                ...
                // 合并结果
                if (t1Result) { suspiciousData.Add("UseDriver"); score += 10; if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:Deep.Driver.a"; }
                if (t2Result) { score += 20; if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:Deep.Virtual.a"; }
                if (t3Result) { suspiciousData.Add("BlackMoon"); score += 15; if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:Deep.BlackMoon.a"; }
                if (t4Result) { suspiciousData.Add("AVKiller"); score += 20; if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:Deep.AVKiller.a"; }
                if (t5Result) { suspiciousData.Add("BugsExploit"); score += 20; if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:Deep.Exploit.a"; }
                if (t6Result) { suspiciousData.Add("SandboxBypass"); score += 20; if (string.IsNullOrEmpty(threatName)) threatName = "HEUR:Deep.SandBypass.a"; }
            }
        ...
        public static async Task<int> CheckExceptionHandling(Xdows.ScanEngine.ScanEngine.PEInfo info)
        {
            return await Task.Run(() =>
            {
